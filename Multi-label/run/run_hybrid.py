...
# conf-my
myconf = Config("conf/hybrid.conf")
...
def train():
    ...
    with tf.device("/gpu:1"):
        for i, k in enumerate(fields):
            ...
            if i == 0:
                plot_model(model, to_file='hybrid.png', show_shapes=True, show_layer_names=False)
...
if __name__ == "__main__":
    if len(sys.argv) != 2 or sys.argv[1] not in ["-train", "-pred"]:
        raise ValueError("""usage: python run_hybrid.py [-train / -pred]""")
