...
def test_pred(x_test,model_path='biGRU'):
    X=pd.read_csv('data/sentiment_analysis_testa.csv')
    model = load_model(model_path, custom_objects={'F1_macro': F1_macro})
...
X = pd.read_csv('data/sentiment_analysis_validationset.csv')
x_train, y_train, x_val, y_val, x_test = get_data(max_num_word=7400, max_len_word=1500, level='char')
resu = model_val(X=X, x_val=x_val, model_path='model/CNN1D_001-8.2334.h5')


test_pred(x_test,model_path='model/CNN1D_002-7.8324.h5')